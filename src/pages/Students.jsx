...
import { predictStudentRisk, explainStudentRisk } from "../services/api";
...
const S = `
  ...
  .explain-text { font-size:11.5px; color:var(--text3); line-height:1.5; max-width:280px; }
  .progress-bar-wrap { width:100%; background:rgba(15,244,198,0.1); border-radius:99px; height:4px; margin-top:6px; overflow:hidden; }
  .progress-bar-fill { height:4px; background:var(--teal); border-radius:99px; transition:width .3s ease; }
`;

const EMPTY = {
  name:"", age:"", failures:"", absences:"", G1:"", G2:"",
  studytime:"", health:"", schoolsup_yes:"0", internet_yes:"1", higher_yes:"1",
  subject1_name:"Math", subject1_attendance:"", subject1_marks:"",
  subject2_name:"Science", subject2_attendance:"", subject2_marks:""
};
...
// Clean 5-field object for AI explanation
function buildExplainData(md) {
  return {
    studytime: md.studytime ?? 2,
    failures:  md.failures  ?? 0,
    absences:  md.absences  ?? 6,
    G1:        md.G1        ?? 10,
    G2:        md.G2        ?? md.G1 ?? 10,
  };
}

// ✅ FileReader instead of file.text() — avoids Buffer/encoding issues
...
// ✅ Robust CSV parser: strips BOM, normalises line endings, handles quoted fields
function parseCSV(rawText) {
  ...
  for (const r of ["name", "subject1_name", "subject1_attendance", "subject1_marks"]) {
    if (!headers.includes(r))
      throw new Error(`Missing required column: "${r}". Check your CSV header row.`);
  }

  return lines.slice(1).map((line, i) => {
    // Character-by-character split that respects quoted commas
    const vals = [];
    let cur = "", inQuote = false;
    for (let c = 0; c < line.length; c++) {
      const ch = line[c];
      if (ch === '"')            { inQuote = !inQuote; }
      else if (ch === "," && !inQuote) { vals.push(stripQuotes(cur)); cur = ""; }
      else                       { cur += ch; }
    }
    vals.push(stripQuotes(cur));

    const row = {};
    headers.forEach((h, idx) => { row[h] = vals[idx] !== undefined ? vals[idx] : ""; });

    if (!row.name) throw new Error(`Row ${i + 2}: "name" column is empty`);

    const subjects = {
      [row.subject1_name || "Math"]: {
        attendance: parseInt(row.subject1_attendance) || 0,
        marks:      parseInt(row.subject1_marks)      || 0,
      }
    };
    if (row.subject2_name && row.subject2_attendance) {
      subjects[row.subject2_name] = {
        attendance: parseInt(row.subject2_attendance) || 0,
        marks:      parseInt(row.subject2_marks)      || 0,
      };
    }

    const g1 = parseInt(row.g1) || parseInt(row.G1) || 10;
    const g2 = parseInt(row.g2) || parseInt(row.G2) || g1;
    ...
    return {
      ...
      modelData: {
        ...
        absences:      parseInt(row.absences)      || 6,
        health:        parseInt(row.health)        || 3,
        G1: g1, G2: g2,
        schoolsup_yes: parseInt(row.schoolsup_yes) || 0,
        internet_yes:  parseInt(row.internet_yes)  || 1,
        higher_yes:    parseInt(row.higher_yes)    || 1,
        Medu:2, Fedu:2, traveltime:2, famrel:3, freetime:3, goout:3, Dalc:1, Walc:1,
        school_MS:0, sex_M:1, address_U:1, famsize_LE3:0, Pstatus_T:1,
        Mjob_health:0, Mjob_other:1, Mjob_services:0, Mjob_teacher:0,
        Fjob_health:0, Fjob_other:1, Fjob_services:0, Fjob_teacher:0,
        reason_home:0, reason_other:0, reason_reputation:1,
        guardian_mother:1, guardian_other:0, famsup_yes:1, paid_yes:0,
        activities_yes:1, nursery_yes:1, romantic_yes:0,
      }
    };
  });
}
...
export default function Students() {
  ...
  const [list,        setList]        = useState(() => {
    try { const s = localStorage.getItem("studentList"); return s ? JSON.parse(s) : []; }
    catch { return []; }
  });
  ...
  // ─── Load predictions on mount — sequential to protect local server ───────────
  useEffect(() => {
    async function run() {
      setLoading(true);
      const result = {};
      for (const s of list) {
        ...
        try {
          const prediction = await predictStudentRisk(md);
          let explanation = "";
          try {
            const expRes = await explainStudentRisk(
              s.name, buildExplainData(md), prediction.risk_class, prediction.risk_level
            );
            explanation = expRes.explanation || "";
          } catch { explanation = ""; }
          result[s.id] = { overall: { ...prediction, explanation } };
        } catch {
          result[s.id] = { overall: { risk_level:"Unknown", probability:null, explanation:"" } };
        }
      }
      setRisks(result);
      setLoading(false);
  ...
  // Fallback model data for legacy records without modelData
  function buildMD(s) {
    const subs = Object.values(s.subjects);
    const avg  = subs.reduce((a, d) => a + d.marks, 0) / subs.length;
    const g1   = Math.round(avg / 5);
    return {
      age:17, Medu:2, Fedu:2, traveltime:2, studytime:2, failures:0, famrel:3,
      freetime:3, goout:3, Dalc:1, Walc:1, health:3, absences:6, G1:g1, G2:g1,
      school_MS:0, sex_M:1, address_U:1, famsize_LE3:0, Pstatus_T:1,
      Mjob_health:0, Mjob_other:1, Mjob_services:0, Mjob_teacher:0,
      Fjob_health:0, Fjob_other:1, Fjob_services:0, Fjob_teacher:0,
      reason_home:0, reason_other:0, reason_reputation:1,
  ...
  // ─── CSV Import ───────────────────────────────────────────────────────────────
  // ✅ KEY FIX: sequential for-loop instead of Promise.all
  //    Promise.all sends ALL requests at once → local FastAPI server hangs/buffers
  //    for-loop sends ONE request at a time → server handles each comfortably
  async function handleCSV(e) {
    ...
    try {
      // Step 1: read file via FileReader (not file.text() — avoids Buffer issues)
      ...
      // Step 2: process one student at a time
      for (let i = 0; i < parsed.length; i++) {
        const s = parsed[i];
        try {
          const prediction = await predictStudentRisk(s.modelData);
          let explanation = "";
          try {
            const expRes = await explainStudentRisk(
              s.name, buildExplainData(s.modelData), prediction.risk_class, prediction.risk_level
            );
            explanation = expRes.explanation || "";
          } catch { explanation = ""; }
          newRisks[s.id] = { overall: { ...prediction, explanation } };
        } catch {
          newRisks[s.id] = { overall: { risk_level:"Unknown", probability:null, explanation:"" } };
        }

        setCsvProgress({ done: i + 1, total: parsed.length });
        setCsvOk(`⏳ Processing ${i + 1} of ${parsed.length}...`);
      }
  ...
  function handleDownload() {
    const rows = [["Name","Subjects","Avg Attendance (%)","Avg Marks","Risk Level","Risk Reason","AI Probability (%)","AI Explanation"]];
    for (const s of list) {
      const subs   = Object.values(s.subjects);
      ...
      const aiH    = r?.risk_level === "High Risk";
      const attLow = avgA < attT;
      const mLow   = avgM < marksT;
      const high   = aiH || attLow || mLow;
      const reasons = [];
      if (aiH)    reasons.push("AI flagged");
      if (attLow) reasons.push("Low Attendance");
      if (mLow)   reasons.push("Low Marks");
      rows.push([
        s.name,
        Object.keys(s.subjects).join(" | "),
        avgA.toFixed(1), avgM.toFixed(1),
        high ? "High Risk" : "Low Risk",
        high ? reasons.join(", ") : "Good Performance",
  ...
  async function handleAdd() {
    setFormError("");
    const required = ["name","age","failures","absences","G1","G2","studytime","health","subject1_attendance","subject1_marks"];
    ...
    const md = {
      ...
      studytime:     parseInt(form.studytime),
      failures:      parseInt(form.failures),
      absences:      parseInt(form.absences),
      health:        parseInt(form.health),
      G1:            parseInt(form.G1),
      G2:            parseInt(form.G2),
      schoolsup_yes: parseInt(form.schoolsup_yes),
      internet_yes:  parseInt(form.internet_yes),
      higher_yes:    parseInt(form.higher_yes),
      Medu:2, Fedu:2, traveltime:2, famrel:3, freetime:3, goout:3, Dalc:1, Walc:1,
      school_MS:0, sex_M:1, address_U:1, famsize_LE3:0, Pstatus_T:1,
      Mjob_health:0, Mjob_other:1, Mjob_services:0, Mjob_teacher:0,
      Fjob_health:0, Fjob_other:1, Fjob_services:0, Fjob_teacher:0,
    ...
    try {
      ...
      try {
        const expRes = await explainStudentRisk(
          form.name, buildExplainData(md), prediction.risk_class, prediction.risk_level
        );
        explanation = expRes.explanation || "";
      } catch { explanation = ""; }
      ...
      const ns = {
        ...
        subjects: {
          [form.subject1_name]: {
            attendance: parseInt(form.subject1_attendance),
            marks:      parseInt(form.subject1_marks),
          },
          ...(form.subject2_attendance ? {
            [form.subject2_name]: {
              attendance: parseInt(form.subject2_attendance),
              marks:      parseInt(form.subject2_marks),
            }
          } : {})
        },
        modelData: md,
      ...
      setRisks(prev => ({ ...prev, [ns.id]: { overall: { ...prediction, explanation } } }));
      setShowForm(false);
      setForm(EMPTY);
    } catch {
      setFormError("Failed to get AI prediction. Is the backend running?");
    }
  ...
  // ─── Combined risk: AI model + threshold rules ────────────────────────────────
  function getRisk(id) {
    const r = risks[id]?.overall;
    const s = list.find(x => x.id === id);
    if (!r || !s) return { level:"...", reason:"Loading...", probability:null, explanation:"" };
    ...
    return {
      level:       high ? "High" : "Low",
      reason:      high ? reasons.join(", ") : "Good Performance",
  ...
  const handleLogout = () => {
    ...
    if (email) {
      localStorage.setItem(`studentList_${email}`, sb);
      localStorage.setItem(`predictionHistory_${email}`, hb);
    }
    window.location.href = "/login";
  };

  const filtered     = list.filter(s => s.name.toLowerCase().includes(search.toLowerCase()));
  const progressPct  = csvProgress.total > 0
        ...
        <div className="scroll-area">
          ...
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6 }}>
              ...
              {/* Progress bar — only visible while importing */}
              {csvLoading && csvProgress.total > 0 && (
                <div style={{ width:"100%", minWidth:260 }}>
                  ...
                  <div style={{ fontSize:11, color:"var(--text3)", marginTop:4, textAlign:"right" }}>
                    {csvProgress.done} / {csvProgress.total} students
                  </div>
                </div>
              )}

              {csvError                  && <div style={{ fontSize:12, color:"var(--red)"   }}>{csvError}</div>}
              {!csvLoading && csvOk      && <div style={{ fontSize:12, color:"var(--green)" }}>{csvOk}</div>}
            </div>
          </div>
          ...
          <div className="csv-hint">
            ...
            {" "}· Optional: <code>age, failures, absences, G1, G2, studytime, health, schoolsup_yes, internet_yes, higher_yes, subject2_name, subject2_attendance, subject2_marks</code>
          </div>
          ...
          {showForm && (
            <div className="form-card">
              ...
              <div className="g4" style={{ marginBottom:20 }}>
                ...
                <Field label="Mid-term Grade G2 (0–20)"  field="G2"        min={0} max={20}  form={form} setForm={setForm} />
          ...
          <div className="card">
            <div className="card-title">
              <span className="dot" />Student Records{" "}
              <span style={{ fontSize:12, color:"var(--text3)", fontWeight:400, fontFamily:"var(--font-body)" }}>
                ({filtered.length})
              </span>
            </div>
                  ...
                  <tbody>
                    {filtered.map(s => {
                      ...
                      return (
                        <tr key={s.id}>
                          <td>
                            <button className="name-link" onClick={() => navigate(`/student/${s.id}`)}>
                              {s.name}
                            </button>
                          </td>
                          <td>
                          ...
                          <td>
                            <span className={`badge ${risk.level === "High" ? "badge-high" : "badge-low"}`}>
                              {risk.level === "High" ? "High Risk" : "Low Risk"}
                            </span>
                          </td>
                          <td style={{
                            fontFamily: "var(--font-disp)",
                            fontWeight: 700,
                            color: risk.probability != null
                              ? (risk.probability > 0.5 ? "var(--red)" : "var(--green)")
                              : "var(--text3)"
                          }}>
                          ...
                          <td>
                            {risk.explanation
                              ? <div className="explain-text">{risk.explanation}</div>
                              : <span style={{ color:"var(--text3)" }}>—</span>}
                          </td>
                          <td>
                            <button className="btn-link" onClick={() => handleDelete(s.id)}>🗑</button>
                          </td>
                        </tr>
                      );
                    })}
