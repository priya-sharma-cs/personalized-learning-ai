...
export default function RiskAlerts() {
  ...
  function getInfo(s) {
    ...
    const riskClass=r?.risk_class??0; // 0=Low, 1=Medium, 2=High
    const aiH=riskClass>=2; const aiM=riskClass===1;
    const attLow=a<attT; const mLow=m<marksT;
    const reasons=[]; if(aiH)reasons.push("ai"); if(aiM)reasons.push("ai-medium"); if(attLow)reasons.push("att"); if(mLow)reasons.push("marks");
    const severity=reasons.filter(r=>r!=="ai-medium").length>=3?"critical":aiH&&(attLow||mLow)?"high":aiH||attLow||mLow?"medium":aiM?"medium":"ok";
    return {isHigh:aiH||attLow||mLow||aiM,reasons,severity,avgA:a,avgM:m,probability:r?.probability??null,riskLevel:r?.risk_level??"Unknown"};
  }
                        ...
                        <div key={s.id} className={`alert-card ${sc.cls}`} style={{ animationDelay:`${i*60}ms` }}>
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div className="alert-reasons">
                              <span style={{ fontSize:11, padding:"3px 8px", borderRadius:20, background:"rgba(255,255,255,0.05)", color:"var(--text3)", fontWeight:600 }}>{info.riskLevel}</span>
                              {info.reasons.includes("ai")        && <span className="chip chip-ai">🤖 AI High Risk</span>}
                              {info.reasons.includes("ai-medium") && <span className="chip chip-ai">🤖 AI Medium Risk</span>}
